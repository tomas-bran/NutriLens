...
describe('<HistoryListView> — empty state (US-26)', () => {
  it('shows the empty state when total === 0', () => {
    render(<HistoryListView items={[]} page={1} totalPages={0} total={0} />);
...
describe('<HistoryListView> — populated grid (US-23)', () => {
  it('renders one card per item', () => {
    ...
    render(<HistoryListView items={items} page={1} totalPages={1} total={3} />);
  ...
  it('every card links to /historial/[id]', () => {
    ...
    render(<HistoryListView items={items} page={1} totalPages={1} total={2} />);
  ...
  it('renders the allergen chips when the array is non-empty', () => {
    ...
    render(<HistoryListView items={items} page={1} totalPages={1} total={1} />);
...
describe('<HistoryListView> — header copy', () => {
  it('shows singular copy when total is 1', () => {
    render(<HistoryListView items={[mkItem()]} page={1} totalPages={1} total={1} />);
  ...
  it('shows plural copy when total > 1', () => {
    ...
    render(<HistoryListView items={items} page={1} totalPages={1} total={42} />);
...
describe('<HistoryListView> — pagination', () => {
  ...
  it('renders pagination when totalPages > 1', () => {
    ...
    render(<HistoryListView items={items} page={1} totalPages={3} total={36} />);
  ...
  it('disables next when on the last page', () => {
    const items = Array.from({ length: 12 }, (_, i) => mkItem({ id: `p-${i}` }));
    render(<HistoryListView items={items} page={3} totalPages={3} total={36} />);
  ...
  it('enables both prev and next on a middle page', () => {
    const items = Array.from({ length: 12 }, (_, i) => mkItem({ id: `p-${i}` }));
    render(<HistoryListView items={items} page={2} totalPages={3} total={36} />);
    expect(screen.getByTestId('history-page-prev')).toHaveAttribute('href', '/historial?page=1');
